...
class _MarkerClusterLayerState extends State<MarkerClusterLayer>
    ...
    return Stack(
      children: [
        ...
        MobileLayerTransformer(
          child: Stack(children: _buildMobileTransformStack()),
        ),
    ...
    for (var i = 0; i < cluster.markers.length; i++) {
      ...
      results.add(
        _buildMarker(
          ...
          fade: Fade.fadeIn(
            curve: widget.options.animationsOptions.fadeInCurve,
          ),
  ...
  List<Widget> _buildMobileTransformStack() {
    ...
    // marker.
    final recursionBounds = _extendBounds(widget.mapCamera.visibleBounds, 0.5);
  ...
  VoidCallback _onClusterTap(MarkerClusterNode cluster) {
    return () async {
      ...
      final latTween = Tween<double>(
        ...
        end: dest.center.latitude,
      );
      final lonTween = Tween<double>(
        begin: center.longitude,
        end: dest.center.longitude,
      );
      final zoomTween = Tween<double>(
        begin: widget.mapCamera.zoom,
        end: dest.zoom,
      );
      ...
      final listener = _centerMarkerListener(
        animation,
        latTween,
        lonTween,
        zoomTween: zoomTween,
  ...
  VoidCallback _onMarkerTap(MarkerNode marker) {
    return () {
      ...
      final latTween = Tween<double>(
        begin: center.latitude,
        end: marker.point.latitude,
      );
      final lonTween = Tween<double>(
        begin: center.longitude,
        end: marker.point.longitude,
      );
...
LatLngBounds _extendBounds(LatLngBounds bounds, double stickonFactor) {
  ...
  final point1 = LatLng(
    (bounds.south - height).clamp(-90, 90),
    (bounds.west - width).clamp(-180, 180),
  );
  final point2 = LatLng(
    (bounds.north + height).clamp(-90, 90),
